...
class ChannelAdapterContractTest {
    ...
    void feishuListOrgStructureAssemblesDepartmentTreeWithUsers() {
        ...
        String deptResp = "{\"code\":0,\"msg\":\"success\",\"data\":{\"items\":["
                + "{\"department_id\":\"D1\",\"name\":\"总公司\",\"parent_id\":\"0\"},"
                + "{\"department_id\":\"D2\",\"name\":\"技术部\",\"parent_id\":\"D1\"}"
                + "],\"has_more\":false}}";
        ...
        server.expect(once(), requestTo("https://open.feishu.cn/open-apis/auth/v3/tenant_access_token/internal")).andExpect(method(HttpMethod.POST)).andRespond(withSuccess(FEISHU_TOKEN_SUCCESS, MediaType.APPLICATION_JSON));
        server.expect(once(), requestTo(startsWith("https://open.feishu.cn/open-apis/contact/v3/departments?"))).andExpect(method(HttpMethod.GET)).andRespond(withSuccess(deptResp, MediaType.APPLICATION_JSON));
        server.expect(once(), requestTo(startsWith("https://open.feishu.cn/open-apis/contact/v3/users/find_by_department?department_id=D1"))).andExpect(method(HttpMethod.GET)).andRespond(withSuccess(d1Users, MediaType.APPLICATION_JSON));
        server.expect(once(), requestTo(startsWith("https://open.feishu.cn/open-apis/contact/v3/users/find_by_department?department_id=D2"))).andExpect(method(HttpMethod.GET)).andRespond(withSuccess(d2Users, MediaType.APPLICATION_JSON));
        RestClient client = builder.build();
